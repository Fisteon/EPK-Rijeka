...
public class TMPFadeInText : MonoBehaviour
{
    ...
    IEnumerator test()
    {
        string x = m_TextComponent.text;
        while (true)
        {
            yield return new WaitUntil(() => m_TextComponent.text != x);
            FadeOnChange();
            x = m_TextComponent.text;
        }
    }

    public void FadeOnChange()
    {
    ...
    {
        Debug.Log("Starting text fading");
            ...
            for (int i = startingCharacterRange; i < currentCharacter + 1; i++)
            {
                ...
                if (alpha == 255)
                {
                    ...
                    if (startingCharacterRange == characterCount)
                    {
                        ...
                        if (next != null)
                        {
                            MainMenuController.Menu.ActivateNextText(next, delay);
